...
---
  ...
  If I need to choose only one programming language to master, Javascript would be my choice. With Javascript we are able to create everything from web applications, mobile apps, and server-side applications, even desktop application using electron.js. I think this should convince you to learn Javascript! In order to get started with Javascript you should know some of its history - that's what this blog is for.
At the time I write this blog post, Javascript has released its 😍ES11(2020)😍 already.

### Below is a brief overview of what is new in all ES versions (<q>click on the list below to jump to its section</q>):
<ul>
...
In 2015, Javascript was back in the game after it had been ignored by most serious programmers ([**Why is Javascript bad?** discussion on quora](https://www.quora.com/Is-JavaScript-really-a-bad-language)).
...
With the old way, we write would write functions as below:
```
...
```
The syntax `()=>{}` is called an arrow function and the constant `sum` is declared to store it. This is also an example of an `anonymous function` because it has no explicit name, but is immediately assigned to a variable.
...
```
> We create new class called `Calculator` (convention encourages the use of a capital letter when creating a class, but it isn't required).
The Calculator class constructor is called whenever the `new` keyword is used to make an instance of the class, and expects one parameter called `moneyBeforeDiscount`.
...
We can also use class inheritance with keyword `extend` and reference back to parent class with the keyword `super`.
...
Template strings replace repetative `+` string operations to concatenate reuglar strings and variables as well as allow multiline string variables. In order to interpolate variables inline just wrap the variable name in `${}`:
...
ES6 also includes the ability to set a `default parameter` which allows developers to set a fallback value if a function is called without necessary parameters.
